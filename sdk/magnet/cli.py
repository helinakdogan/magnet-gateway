...
def _claude_md_block(user_id: str) -> str:
    return "\n".join([
        ...
        "",
        "If the user types *projects, call `list_projects` and present the menu — it lists",
        "personal projects under 'Individual:' and, if the user belongs to any teams, each",
        "team's shared projects grouped under 'Team <name> (<team_id>):'. Picking a personal",
        "project calls set_active_context(project=<chosen>); picking a team project calls",
        "set_active_context(project=<chosen>, team_id=<that team's id>) — remember, *continue,",
        "*memory, and *forget then all operate on that team project directly, subject to the",
        "team's write-permission/approval rules.",
        "If the user types *remember, call `save_now` (pass ALL messages so far) to do a",
        "cumulative save of everything up to this point. (Distinct from the automatic",
        "`remember` tool call above — that one fires per-statement in the background; *remember",
        "is the user-triggered manual cumulative save.)",
        "If the user types *usage, call `get_status` to show memory status and usage.",
        ...
        "If the user asks where they left off / what were we doing / types *continue,",
        ...
        "*team new <name> → call create_team. Teammates join by email invite from the",
        "dashboard's Team tab — there is no manual join-by-id command anymore.",
        "*teams → call list_teams: shows every team the user is in, their shared projects,",
        "and the member list. Leads see full detail (role + per-project access) for every",
        "member; plain members see only other members' names — never their role or",
        "permissions. This is enforced by the tool itself — never guess or add detail it",
        "didn't return.",
        "When the active project is a team project (activated via *projects, see above),",
        "remember/*remember write to team memory directly — no separate 'share' step exists",
        "anymore. A write may come back pending review instead of saved, if a lead restricted",
        "writes for that member/category; that's normal, not an error.",
        "recall, show_project_memory, and *continue automatically merge team memory for a",
        "team-active project. Team items are labeled [team] so the user can see what's",
        "shared vs personal.",
